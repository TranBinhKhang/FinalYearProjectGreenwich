...
function Comment({ userName, comment, onPress, postedAt, isFlagged, onPress2, onReport, showReplyText, deleteShowConditions, onDelete, show, isCorrect, onShowOptions }) {
  return (
    <View style={isFlagged ? styles.flaggedcontainer : styles.outercontainer}>
      ...
      <View style={{
        ...
        {onShowOptions && <View style={{borderWidth: 1, height: '90%', width: '10%', right: '37%', flexDirection: 'row', alignSelf: 'center', justifyContent: 'center', alignContent: 'center', alignItems: 'center'}}>
          <TouchableWithoutFeedback onPress={onShowOptions}><Text style={{color: '#841584'}}>•••</Text></TouchableWithoutFeedback></View>}
        
        <TouchableWithoutFeedback onPress={onPress2}><Text style={ isFlagged ? {color: 'red', marginRight: 7} : {color: '#841584', marginRight: 7}}>{showReplyText}</Text></TouchableWithoutFeedback>
        <Button color={isFlagged ? 'red' :"#841584" } title='Reply ' onPress={onPress}/>
      ...
      {show && <View style={{
        ...
        <Button color="#841584" title='Report Comment' onPress={onReport}/>
